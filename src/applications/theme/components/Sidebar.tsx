...
import { Search, Palette, Star, Sparkles, Folder, Menu, X, Plus } from 'lucide-react';
import { cn } from '../../../utils/cn';

export function Sidebar() {
  const { activeCategory, setActiveCategory, searchQuery, setSearchQuery } = useThemeStore();
  ...
  return (
    <>
      <button 
        className="md:hidden absolute top-3 left-3 z-50 p-2 bg-black/50 rounded-lg text-white"
      ...
      <div className={cn(
        "absolute md:relative z-40 w-64 h-full border-r border-os-window-border bg-os-window-bg/95 flex flex-col transition-transform duration-300 backdrop-blur-os",
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          <div>
            ...
            <div className="space-y-1">
              ...
              >
                <Palette size={16} className={activeCategory === 'All' ? "text-white" : "text-os-text-muted"} />
                All Themes
              </button>
              <button
              ...
              >
                <Star size={16} className={activeCategory === 'Favorites' ? "text-white" : "text-rose-500"} />
                Favorites
              </button>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between px-2 mb-2">
              <h3 className="text-xs font-medium text-os-text-muted uppercase tracking-wider">Categories</h3>
            </div>
            <div className="space-y-1">
